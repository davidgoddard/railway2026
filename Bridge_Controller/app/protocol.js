...
function parseRow(line) {
  ...
  if (p[0] === 'CONFIG' && MAC.test(p[1])) return { type: 'config', mac: p[1], revision: +p[2], count: +p[3], settings: { resolution: +p[4], brightness: +p[5], contrast: +p[6], saturation: +p[7], vflip: +p[8], hmirror: +p[9] } };
  if (p[0] === 'CELL') return { type: 'cell', cell: { id: +p[2], group: +p[3], x: +p[4], y: +p[5], radius: +p[6], shape: +p[7], floor: +p[8], tolerance: +p[9], threshold: +p[10], enter: +p[11], clear: +p[12] } };
...
function commandsForConfig(mac, config) {
  ...
  return [
    ...
    ...config.cells.map((c, i) => `CELL ${mac} ${i} ${c.id} ${c.group} ${c.x} ${c.y} ${c.radius} ${c.shape} ${c.floor} ${c.tolerance} ${c.threshold} ${c.enter} ${c.clear}`),
    `COMMIT ${mac}`,
