...
The first hardware experiment is [the Arduino camera angle sketch](../experiments/camera_module/README.md). It accepts blank or textured background references and historically measured 18 gradient-angle bins with up to three dominant directions. The production camera now measures 36 five-degree bins and retains up to ten supported directions. For an oriented reference, live frames count strong gradients near each stored direction and in an "other" bucket without calculating an angle for each pixel; the change score compares those normalized bucket shares. If the textured reference has no stable peak, it compares normalized angle distributions. The blank/texture cutoff, direction support, tolerance, and persistence periods remain provisional and need empty-track and rolling-stock tests.

Angle proportions alone can miss a narrow object against a weakly patterned background: a carpet test produced a 0.240 angle mismatch while the previous event threshold was 0.30. That separate historical experiment also compared strong-gradient concentration and used a 0.20 mismatch threshold with three-frame persistence. The bridge-connected production camera now defaults new sensors to a 0.40 mismatch threshold and one qualifying frame to occupy; its angle-based detector does not use the experiment’s edge-tail score. Qualify this combination against sudden lighting changes and rolling stock before adopting it in production.

At resolutions where sleepers cannot be resolved, a rail and a carriage can both produce long edges with similar angles. The production detector preserves up to ten supported directions so weaker sleeper, ballast, and surface angles can distinguish the empty track from rolling stock while remaining based on normalized gradient proportions. The setup application applies the same selection rules to a fetched frame and shows a nearest-neighbour cell crop with physical line directions overlaid. This lets an operator move or resize a sensor until it contains useful secondary texture. The separate historical experiment tested saved edge locations; production does not use them because small camera movements caused avoidable sensitivity.
