...
One full-frame clipping pass, two pixel passes per cell, and up to eight extra comparisons for a cell whose first score would trigger can reduce frame rate. The loop starts another capture as soon as its previous frame and pending work are complete. Measure the actual board, resolution, and cell set before using it for traffic decisions. A carriage whose edges have the same angle proportions as the empty track can still look like the baseline.
