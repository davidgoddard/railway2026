...
Firmware `0.2.0` is the current detector. It uses nine fixed physical-line direction buckets centred at 0°, 20°, …, 160°, with three coarse position bands per direction, and combines their distribution change with proportional edge-count change. See the [fixed-angle detector notes](../Documentation/fixed-angle-histogram-experiment.md). A blank reference can still detect newly introduced texture, and exact edge locations are not stored.
...
One full-frame clipping pass, two pixel passes per cell, nine fixed-direction comparisons, and up to eight extra comparisons for a cell whose first score would trigger can reduce frame rate. The loop starts another capture as soon as its previous frame and pending work are complete. Measure the actual board, resolution, and cell set before using it for traffic decisions. Fixed direction boundaries can still amplify small angle movements between adjacent buckets, so test ordinary lighting and representative rolling stock before relying on a threshold.
...
| `CELL_ANALYSIS` | 18 | camera → bridge | sensor/revision/frame, score/threshold, baseline/live edge totals, nine fixed line angles, baseline buckets, and live buckets |
